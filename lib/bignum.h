...
void __bignum_long_div(bignum* n, unsigned d, bignum* q, bignum* r) {
    ...
    for (int i = n->size - 1; i >= 0; i--) {
        remainder = remainder * BASE + n->digits[i];
...
unsigned v_lshift(bignum* a, int d) {
    unsigned carry = 0;
    unsigned shift = pow10(d);
    for (int i = 0; i < a->size; i++) {
        long acc = (long) a->digits[i] * shift + carry;
        a->digits[i] = acc % BASE;
        carry = acc / BASE;
    }
    return carry;
...
unsigned v_rshift(bignum* a, int size, int d) {
    ...
    for (int i = size - 1; i >= 0; i--) {
        unsigned long acc = (unsigned long)carry * BASE + a->digits[i];
        carry = acc % shift;
        a->digits[i] = (unsigned int) (acc / shift);
    }
    return carry;
}
...
void __bignum_div_mod(bignum* v1, bignum* w1, bignum* quotient, bignum* prem) {
    ...
    v0 = (unsigned int*) &v->digits[0];
    w0 = (unsigned int*) &w->digits[0];
    wm1 = w0[w->size - 1];
    wm2 = w0[w->size - 2];

    for (vk = v0 + k, ak = (unsigned int*) &a->digits[0] + k; vk-- > v0;) {
        // fprintf(stderr, "v = %s\n", bignum_to_string(v));
        ...
        vtop = vk[w->size];
        assert(vtop <= wm1);
        vv = ((unsigned long)vtop * BASE) + vk[w->size-1];
        q = (unsigned int)(vv / wm1);
        r = (unsigned int)(vv - (long)wm1 * q); /* r = vv % wm1 */
        while ((unsigned long)wm2 * q > (((unsigned long)r * BASE) + vk[w->size-2])) {
            --q;
            r += wm1;
            if (r >= BASE) {
                break;
